...
The **owner-confirmed canonical classification rule** is the `receipt_name` prefix convention
on `krypton_woosoo.menus` rows:
...
**What the running app actually uses:** `groupAllowedMenusByCategoryCode` and
`groupAllowedMenuPreviews` in `tablet-ordering-pwa/utils/packageModifierGroups.ts`
(lines ~109/~130), imported by `tablet-ordering-pwa/components/PackageCard.vue:5` and
...
only referenced by `tablet-ordering-pwa/tests/package-modifier-groups.spec.ts` and never
imported by any live component.

**Known gap — `meat_category_code` drift (see G2):** `package_allowed_menus.meat_category_code`
is a free-text field entered by the admin (`StorePackageRequest`/`UpdatePackageRequest`,
`app/Http/Controllers/Admin/PackageController.php`) with no server-side derivation from
`receipt_name` and no validation that the two agree. The `receipt_name`-prefix rule is the
target alignment; production grouping runs entirely on the admin-entered code today.
...
| # | Gap | Location | Impact |
|---|---|---|---|
| G1 | **Partially resolved.** For DB-backed non-meats slugs, `categoryMenus` now reads from the `tablet_category_menu` pivot when active categories exist (`hasActiveDbCategories` guard). The `meats` slug always resolves via POS group 34 directly, bypassing the pivot. Hardcoded group-ID fallback (`resolveLegacyCategoryMenus`) applies only when no active DB categories exist. | `TabletApiController::categoryMenus`, `::hasActiveDbCategories`, `tablet_category_menu` | Admin curation is now respected for non-meats DB categories. The `meats` slug and the bootstrap fallback path remain hardcoded. |
| G2 | Production meat grouping runs entirely on admin-entered `meat_category_code` (no server-side derivation from `receipt_name`). The `receipt_name`-prefix classifier (`classifyPackageModifier` et al.) is **dead code** — unit-test only, never imported by any live component. If an admin enters a code that does not match the `receipt_name` prefix, the PWA silently misrepresents the POS classification. | `package_allowed_menus.meat_category_code`, `StorePackageRequest`, `UpdatePackageRequest`, `packageModifierGroups.ts` | No server-side enforcement; gap between owner-confirmed canonical rule and live behavior. |
