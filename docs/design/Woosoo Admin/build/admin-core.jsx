        ...
        <div style={{overflowY:'auto',padding:'16px 18px',display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{display:'flex',gap:14,alignItems:'center'}}>
            <div style={{width:64,height:64,borderRadius:'var(--r-l)',overflow:'hidden',background:'var(--bg3)',border:'1px solid var(--bdr2)',flexShrink:0}}>
              {form.img ? <img src={(window.__resources||{})[form.img]} style={{width:'100%',height:'100%',objectFit:'cover'}} alt=""/> : <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28}}>🍖</div>}
            </div>
            <div style={{flex:1}}>
              ...
              <div key={item.id} className={`card mcard${!item.avail?' unavail':''}`}>
                <div style={{display:'flex',gap:10,marginBottom:9}}>
                  <div className="m-thumb">
                    {item.img ? <img src={`images/food/${item.img}`} alt={item.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <span style={{fontSize:20}}>🍖</span>}
                  </div>
