...
<template>
    ...
    <!-- WOOSOO STEP 4: removed double-wrapped outer container; sections use inner-card
         pattern matching Dashboard; inline 2-col stats replace StatsCards (avoids 4-col grid gap) -->
                ...
                <div class="flex flex-col gap-5">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        <div class="max-w-2xl space-y-1.5">
                            <span class="inline-flex rounded-full border border-border/70 bg-accent/12 px-3 py-1 text-[11px] font-semibold tracking-[0.22em] text-muted-foreground uppercase">Device management</span>
                            ...
                            <p class="max-w-xl text-sm leading-6 text-muted-foreground sm:text-base">
                                Register a new device, review devices that are already online, and rotate security codes when a tablet needs to be re-issued.
                            </p>
                        </div>
                    ...
                    <!-- Inline 2-col stats (avoids StatsCards 4-col empty gap) -->
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:w-1/2">
                        <div class="rounded-[18px] border border-black/8 bg-white/72 px-4 py-4 dark:border-white/10 dark:bg-white/[0.06]">
                            ...
                            <p class="mt-2 text-2xl font-semibold tracking-tight tabular-nums">{{ (devices || []).length }}</p>
                        ...
                        <div class="rounded-[18px] border border-black/8 bg-white/72 px-4 py-4 dark:border-white/10 dark:bg-white/[0.06]">
                            ...
                            <p class="mt-2 text-2xl font-semibold tracking-tight tabular-nums">{{ securityReadyCount }}</p>
