...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <Card
            class="overflow-hidden border-black/8 bg-white/80 shadow-[0_24px_70px_-42px_rgba(37,37,37,0.32)] dark:border-white/10 dark:bg-white/[0.05]"
        >
            <CardHeader class="border-b border-black/8 px-6 py-5 dark:border-white/10">
                <CardTitle class="text-2xl font-semibold tracking-tight text-foreground"> Accessibility </CardTitle>
            ...
            <CardContent class="space-y-8 px-6 py-6">
                ...
                <div v-if="selectedRole" class="grid gap-8">
                    <div v-for="(permissions, groupName) in props.groupedPermissions" :key="groupName" class="space-y-3">
                        <h3 class="flex items-center gap-2 text-lg font-semibold text-foreground capitalize">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                            <div
                                ...
                                :key="permission.id"
                                class="flex items-center justify-between rounded-[22px] border border-black/8 bg-white/82 p-4 transition hover:border-[#f6b56d]/30 hover:shadow-[0_18px_44px_-34px_rgba(37,37,37,0.42)] dark:border-white/10 dark:bg-white/[0.06]"
                            >
                                ...
                                <Switch
                                    ...
                                    :model-value="permissionsState.includes(permission.name)"
                                    @update:model-value="handleTogglePermission(permission.name, $event)"
                    ...
                    <div class="sticky bottom-4 flex justify-end">
                        <Button @click.prevent="savePermissions" :disabled="isSubmitting" class="rounded-2xl px-6 py-3"> Save changes </Button>
                    </div>
                </div>

                <div
                    v-else
                    class="rounded-[22px] border border-dashed border-black/10 bg-black/[0.02] p-6 text-sm leading-6 text-muted-foreground dark:border-white/10 dark:bg-white/[0.04]"
                >
